...
JNIEXPORT jint JNICALL Java_ai_rapids_cudf_Cuda_getComputeCapabilityMajor(JNIEnv *env, jclass) {
  try {
    ...
    int attribute_value;
    CUDF_CUDA_TRY(
        ::cudaDeviceGetAttribute(&attribute_value, cudaDeviceAttributeComputeCapabilityMajor, device));
...
JNIEXPORT jint JNICALL Java_ai_rapids_cudf_Cuda_getComputeCapabilityMinor(JNIEnv *env, jclass) {
  try {
    ...
    int attribute_value;
    CUDF_CUDA_TRY(
        ::cudaDeviceGetAttribute(&attribute_value, cudaDeviceAttributeComputeCapabilityMinor, device));
    return attribute_value;
  }
  CATCH_STD(env, -2);
}
