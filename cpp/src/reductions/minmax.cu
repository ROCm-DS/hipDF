...
struct minmax_binary_op
  ...
  __host__ __device__ minmax_pair<T> operator()(minmax_pair<T> const& lhs, minmax_pair<T> const& rhs) const
  {
...
struct minmax_functor {
    ...
    cudf::column_view const& col, rmm::cuda_stream_view stream, rmm::mr::device_memory_resource* mr)
  {
    ...
    CUDF_CUDA_TRY(hipMemcpyAsync(
      &host_result, dev_result.data(), sizeof(OutputType), hipMemcpyDefault, stream.value()));
