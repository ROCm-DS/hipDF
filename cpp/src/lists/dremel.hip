                         ...
                         rmm::cuda_stream_view stream)
{
  ...
  auto get_empties = [&](column_view col, size_type start, size_type end) {
    ...
    auto empties_idx_end =
      thrust::copy_if(/*rmm::exec_policy(stream)*/thrust::device,
                      thrust::make_counting_iterator(start),
                      thrust::make_counting_iterator(end),
                      empties_idx.begin(),
                      [d_off] __device__(auto i) { return d_off[i] == d_off[i + 1]; });
    auto empties_end = thrust::gather(/*rmm::exec_policy(stream)*/thrust::device,
                                      empties_idx.begin(),
                                      empties_idx_end,
  ...
  size_type curr_rep_values_size = 0;
  {
    ...
    auto ends = thrust::merge_by_key(/*rmm::exec_policy(stream)*/thrust::device,
                                     empties.begin(),
    ...
    thrust::exclusive_scan(
      /*rmm::exec_policy(stream)*/thrust::device, scan_it, scan_it + offset_size_at_level, scan_out.begin());
    ...
    thrust::for_each_n(/*rmm::exec_policy(stream)*/thrust::device,
    ...
    thrust::scatter(/*rmm::exec_policy(stream)*/thrust::device,
                    scatter_it,
