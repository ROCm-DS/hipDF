...
using probe_scheme              = hipco::experimental::linear_probing<1, vocab_hasher>;
...
struct tokenize_vocabulary::tokenize_vocabulary_impl {
  ...
  auto get_map_ref() const { return vocabulary_map->ref(hipco::experimental::op::find); }
...
struct key_pair {
  ...
  {
    return hipco::make_pair(idx, idx);
  }
};
