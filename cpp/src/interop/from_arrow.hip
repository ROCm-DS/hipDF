...
data_type arrow_to_cudf_type(arrow::DataType const& arrow_type)
{
  switch (arrow_type.id()) {
    ...
    case arrow::Type::DECIMAL: {
      // TODO(HIP/AMD): Reenable when we have support for 128 type
      // auto const type = static_cast<arrow::Decimal128Type const*>(&arrow_type);
      // return data_type{type_id::DECIMAL128, -type->scale()};
      auto const type = static_cast<arrow::DecimalType const*>(&arrow_type);
...
std::unique_ptr<column> dispatch_to_cudf_column::operator()<numeric::decimal128>(
  ...
  rmm::mr::device_memory_resource* mr)
{
  //: TODO(HIP/AMD): remove this warning when DECIMAL128 is available
  printf("CAUTION: dispatching type numeric::decimal128 to a cudf column. DECIMAL128 is currently not supported in hipDF!");
  ...
  rmm::mr::device_memory_resource* mr)
{
  ...
  return make_structs_column(
    array.length(), std::move(child_columns), array.null_count(), std::move(out_mask), stream, mr); //TODO(HIP/AMD): is std::move correct for child_columns?
}
