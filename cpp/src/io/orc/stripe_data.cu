    ...
    if (t >= num_vals_read and t < num_vals_to_read) {
      ...
      auto const scaled_value = [&]() {
        ...
        if (scale >= 0) {
          ...
          auto const abs_scale = min(-scale, 27);
#if 0 //: TODO(HIP/AMD): leads to invalid libcall legalization error, see https://ontrack-internal.amd.com/browse/SWDEV-426193
          return (v / kPow5i[abs_scale]) >> abs_scale;
#endif //: TODO(HIP/AMD): leads to invalid libcall legalization error, see https://ontrack-internal.amd.com/browse/SWDEV-426193
          printf("Error: Trying to decode a decimal using int128_t division, which is not supported with hipclang yet. See https://ontrack-internal.amd.com/browse/SWDEV-426193\n");
        }
      }();
  ...
  if (is_nulldec) {
    ...
    if ((encoding_kind == DICTIONARY || encoding_kind == DICTIONARY_V2) &&
      ...
      while (s->top.dict.dict_len > 0) {
        uint32_t numvals        = min((int)s->top.dict.dict_len, blockDim.x), len;
                                          ...
                                          Storage& temp_storage)
{
  ...
  if (t == 0) {
    if (s->chunk.skip_count != 0) {
      s->u.rowdec.nz_count = min(min((int)s->chunk.skip_count, s->top.data.max_vals), blockDim.x);
  ...
  while (s->u.rowdec.nz_count < s->top.data.max_vals &&
    ...
    uint32_t nrows = min((int)s->top.data.end_row - (s->top.data.cur_row + s->top.data.nrows),
                         min((int)(row_decoder_buffer_size - s->u.rowdec.nz_count) * 2, blockDim.x));
  ...
  if (t == 0 and is_valid) {
    ...
    if (num_rowgroups > 0) {
      ...
      uint32_t rowgroup_rowofs =
        (level == 0) ? (blockIdx.y - min((int)s->chunk.rowgroup_id, blockIdx.y)) * rowidx_stride
