 ...
 */
  // FIXME(HIP/AMD): with the template kernel, we would run into the error "reproducer_struct.hip:16:30: error: initialization is not supported for __shared__ variables.
