...
std::vector<cudf::packed_table> do_chunked_pack(cudf::table_view const& input)
{
  ...
  while (chunked_pack->has_next()) {
    ...
    static_cast<void>(hipMemcpyAsync((uint8_t*)final_buff.data() + final_buff_offset,
                    bounce_buff.data(),
                    bytes_copied,
                    hipMemcpyDefault,
...
TEST_F(ContiguousSplitTableCornerCases, OutBufferToSmall)
{
  ...
  // TODO(HIP/AMD): remove (void)?
  EXPECT_THROW((void)cudf::chunked_pack::create({}, 1 * 1024), cudf::logic_error);
}
